    ...
    try {
      ...
      if (bindingResult.hasErrors()) {
        return CommonResult.validateFailed(
            "参数校验不通过，" + Objects.requireNonNull(bindingResult.getFieldError()).getDefaultMessage());
      }
      DynamicInfo dynamicInfo =
          this.dynamicInfoService.findDynamicInfoByPrimaryKey(commentDTO.getDynamicInfoId());
      if (Objects.isNull(dynamicInfo)) {
        return CommonResult.validateFailed("参数校验不通过，动态内容信息不存在。");
      }
      // 校验发布的内容是否包含敏感词汇
      SensitiveWords sensitiveWords =
          this.sensitiveWordsService.checkHasSensitiveWords(commentDTO.getContent());
      if (!Objects.isNull(sensitiveWords)) {
        return CommonResult.validateFailed(
            "参数校验不通过，评论内容包含" + sensitiveWords.getTypeName() + "类型敏感词汇，不允许发布。");
      }
