...
public class DynamicServiceImpl implements DynamicService {
	...
	public int insert(User user, Dynamic dynamic, DynamicInfo dynamicInfo, AttacheInfo attacheInfo) throws Exception {
		...
		log.info("user={}, dynamic={}, dynamicInfo={},attacheInfo={}", user, dynamic, dynamicInfo, attacheInfo);
	...
	public String ajaxUploadExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (int i = 0; i < listob.size(); i++) {
			...
			if (!lo.isEmpty() && StrUtil.isNotEmpty(lo.get(0).toString()) && lo.size() == 24) {
				...
				String weixinIdString = String.valueOf(lo.get(2)); // 微信号
				...
				String proviceString = String.valueOf(lo.get(9)); // 注册定位（省份）
				...
				if (StrUtil.isNotEmpty(dir)) {
					...
					if (StrUtil.isNotEmpty(weixinIdString)) {
						user.setWeixinId(weixinIdString);// 微信号
					} else {
					...
					if (StrUtil.isNotEmpty(proviceString)) {
						user.setProvince(proviceString);// 注册定位（省份）
					}
					...
					String dynamicProviceString = String.valueOf(lo.get(12)); // 动态定位地址（省份）
					...
					if (StrUtil.isNotEmpty(dynamicProviceString)) {
						dynamic.setProvince(dynamicProviceString);
					} else {
					...
					String contentStatuString = String.valueOf(lo.get(16));// 动态内容状态
					String publishStatuString = String.valueOf(lo.get(17));// 是否公开定位状态
					...
					dynamicInfo.setDynamicStatus(contentStatuString);
					dynamicInfo.setPublicStatus(publishStatuString);
